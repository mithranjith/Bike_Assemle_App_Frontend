    ...
    return (
        ...
        >
            {items.map((item, index) => (
                <button
                    className={twMerge(
                        `relative z-1 group flex justify-center items-center hover:text-purplle ${
                            visible
                                ? `flex w-full h-16 rounded-xl bg-n-6 md:w-8 md:h-8 md:mx-auto ${
                                      item.active && "hidden"
                                  }`
                                : `h-10 basis-1/2 base2 font-semibold text-n-4 transition-colors  ${
                ...
                >
                    <Icon
                        className={`fill-n-4 transition-colors group-hover:fill-n-1 hover:text-purplle ${
                            !visible && "mr-3"
